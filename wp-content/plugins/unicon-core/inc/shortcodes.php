	...
	function minti_recentposts_function($atts){
		...
		if( $wp_query->have_posts() ) :
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
		  		...
		  		if(isset($blog_thumbnail[0]) && $blog_thumbnail[0] != ''){
							...
							'.$return3.'
						  	<div class="blog-list-item-description">
								<h3><a href="'.esc_url(get_permalink()).'" title="' . esc_attr(get_the_title()) . '">'.esc_html(get_the_title()) .'</a></h3>
							  	<div class="blog-list-item-excerpt">'.$return2.'.. <a href="'. esc_url(get_permalink()) . '">' .__( 'Read More &rarr;', 'unicon-core' ) . '</a></div>
							</div>
					  	</div>';
...
function minti_googlemaps_function($attr) {
	...
	$getScheme = NULL;
	if($attr['colorscheme'] != '') {
		$getScheme = 'styles: '.rawurldecode(base64_decode($attr['colorscheme'])).','; // Theme Check: When user inserts a custom google maps style, this is required to decode the style.
	}
				...
				if (status == google.maps.GeocoderStatus.OK) {
					...
					if ($attr['marker'] !='')
					{
						...
						if($attr['infowindow'] != '') 
						{
							...
							$thiscontent = wp_specialchars_decode(preg_replace( "/\r|\n/", "", $attr['infowindow'])); // HTML allowed, no escaping
							$returnme .= '
...
if(function_exists( 'minti_portfolio_register' )){ 
	if(!function_exists('minti_portfolio_function')) {
				...
				<div class="container">
					<div class="sixteen columns clearfix">
						<?php if($portfolio_filters): ?>
							<ul class="clearfix">
								<li><a href="#" data-filter="*" class="filter-all active"><?php _e('All', 'unicon-core'); ?></a></li>	
		...
		<?php if($pagination == 'yes'){ ?>
			<div class="pagination-<?php echo esc_attr($style); ?>"><?php get_template_part( 'framework/inc/nav' ); ?></div>
		<?php } ?>
...
if (!function_exists('minti_blog_slider_function')) {
    function minti_blog_slider_function($atts, $content = null) {
        ...
        if($blog_query->have_posts()) :
                ...
                $html .= '<li>';
                	$html .= '<div class="blogslider_post_holder">';
                		...
                		$html .= '<div class="blogslider_text_wrapper"><div class="blogslider_text_outer"><div class="blogslider_text_inner"><div class="blogslider_text_inner2">';
                			...
                			$html .= '<a href="'.get_permalink().'" class="button medium color-3">'.__('read more', 'unicon-core').'</a>';
                			$html .= '</div>';
            ...
            $html .= '<p>'.esc_html__('Sorry, no posts matched your criteria.', 'unicon-core').'</p>';
        endif;
...
/*-----------------------------------------------------------------------------------*/
/* Provide Basic functionality if main Theme does not exist.
/*-----------------------------------------------------------------------------------*/
if(!function_exists('minti_custom_excerpt') && !defined( 'MINTI_THEME_NAME')) {
...
// Word Limiter
if(!function_exists('minti_limit_words') && !defined( 'MINTI_THEME_NAME')) {
...
if(!function_exists('minti_hex2rgba') && !defined( 'MINTI_THEME_NAME')) {
